...
%}
function Licking_Optogenetics2 % name of the file and function must be the same. Put the file in the Protocol folder that also has the same name.
...
MaxTrials = nReward+nCueReward+nRandomTrials; % number of total trials
...
mixTrials(mixTrials>=0.4) = 1; % 60% of trials - no laser
mixTrials(mixTrials< 0.4 & mixTrials >= 0.2) = 2; % 20% of trials - laser
mixTrials(mixTrials< 0.2) = 3;
% mixTrials(mixTrials>=0.5) = 1; % 70% of trials - no laser
% mixTrials(mixTrials< 0.5) = 2;

TrialTypes = [ones(1,nReward), ones(1,nCueReward)*2, mixTrials]; % ones(1,nReward) = [1 1 1 1 1], ones(1,nCueReward)*2 = [2 2 2 2 2]
...
for currentTrial = 1:MaxTrials
    ...
    switch TrialTypes(currentTrial) % Determine trial-specific state matrix fields
        ...
        case 3
            StartState = 'CueLaser'; % Laser during Cue 
    end
    ...
    sma = SetGlobalTimer(sma, 'TimerID', 2, 'Duration', 5, 'OnsetDelay', 0,...
        'Channel', 'BNC2', 'OnLevel', 1, 'OffLevel', 0);
    
    sma = SetGlobalTimer(sma, 'TimerID', 3, 'Duration', 2, 'OnsetDelay', 0,...
        'Channel', 'BNC2', 'OnLevel', 1, 'OffLevel', 0);
    ...
    sma = AddState(sma, 'Name', 'TrialStart', ...
        ...
        sma = AddState(sma, 'Name', 'CueLaser', ...
        'Timer', S.GUI.CueDuration,...
        'StateChangeConditions', {'Tup', 'Reward'},...
        'OutputActions', {'PWM1', 10,'GlobalTimerTrig', '100'}); % StartState 1 or 2
    
    sma = AddState(sma, 'Name', 'Cue', ...
        'Timer', S.GUI.CueDuration,...
        'StateChangeConditions', {'Tup', 'Delay'},...
        'OutputActions', {'PWM1', 10});  % LED light cue (1sec)
    
    sma = AddState(sma, 'Name', 'Delay', ...
        'Timer', S.GUI.RewardDelay,...
        'StateChangeConditions', {'Tup', Reward},...
        'OutputActions', {});   % Delay (1sec)
    
    % water reward without laser
    ...
    % water reward with laser
    sma = AddState(sma, 'Name', 'Laser', ...
        'Timer', 0,...
        'StateChangeConditions', {'Tup', 'Reward2'},...
        'OutputActions', {'GlobalTimerTrig', '10'}); % trigger BNC2 for 5 sec
    
    sma = AddState(sma, 'Name', 'Reward2', ...
        'Timer', ValveTime,...
        'StateChangeConditions', {'Tup', 'Drinking'},...
        'OutputActions', {'ValveState', 1}); % {'ValveState', 1, 'PWM1', 10} to have two outputs
